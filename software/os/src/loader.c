...
static uintptr_t elf_load(const char *filename)
{
    ...
    Elf_Ehdr elf_h;
    // size_t num = sizeof(file_table[]) / sizeof(file_table[0]);

    int idx = fs_open(filename, 0, 0);
    assert(idx!= -1);
    size_t offset = file_table[idx].disk_offset;
    ramdisk_read(&elf_h, offset, sizeof(Elf_Ehdr));
    // assert(*((int *)(elf_h.eindent)) == 0x464C457F);
    ...
    for (int i = 0; i < elf_h.e_phnum; i++)
    {
        ...
        ramdisk_read(&elf_ph, offset, sizeof(Elf_Phdr));
        if (elf_ph.p_type == PT_LOAD)
        {
            ramdisk_read((void*)elf_ph.p_vaddr, elf_ph.p_offset+file_table[idx].disk_offset, elf_ph.p_filesz);
