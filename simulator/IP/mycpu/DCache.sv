...
/* -------------- 8 read data control: choose data from mem or return buffer -------------- */
    ...
    assign rdata_ret    = uncached ? {ret_buf[BIT_NUM-33:0], ret_buf[BIT_NUM-1:BIT_NUM-32]} : ret_buf >> {addr_pipe[BYTE_OFFSET_WIDTH-1:2], 5'b0};
...
/* -------------- 12 miss address buffer: store the address of the cache line to be write -------------- */
    always_ff @(posedge clk) begin
        if(!rstn) begin
            ...
            maddr_buf <= uncached ? addr_pipe & ~32'b11 :{
...
/* -------------- 14 memory settings -------------- */
    ...
    assign d_rlen   = uncached ? 0 : WORD_NUM - 1;
    assign d_waddr  = maddr_buf;
    assign d_wsize  = 3'h2;
    assign d_wlen   = uncached ? 0 : WORD_NUM - 1;
    ...
    always_comb begin
        ...
        INIT: begin
            if(wfsm_en) begin
                ...
                if(uncached&&rvalid_pipe) 
                    wfsm_next_state = FINISH;
                else if(uncached&&wvalid_pipe)
