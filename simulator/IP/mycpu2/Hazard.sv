...
module Hazard(
    ...
    always_comb begin
        ...
        flush_by_load_use = is_load_ex & (rf_rd_ex == rf_rs1_id | rf_rd_ex == rf_rs2_id);
    end
    ...
    wire flush_by_ecall_ex = ecall_ex;
    wire flush_by_ecall_wb = ecall_wb;
    wire flush_by_mret = mret_ex;

    // Lab3 TODO: generate pc_set, IF1_IF2_flush, IF2_ID_flush, ID_EX_flush, EX_LS_flush, LS_WB_flush
    assign pc_set           = flush_by_jump |flush_by_csr|flush_by_ecall_ex|flush_by_ecall_wb|flush_by_mret;
    assign IF1_IF2_flush    = flush_by_jump |flush_by_csr|flush_by_ecall_ex|flush_by_ecall_wb|flush_by_mret;
    assign IF2_ID_flush     = flush_by_jump |flush_by_csr|flush_by_ecall_ex|flush_by_ecall_wb|flush_by_mret;
    assign ID_EX_flush      = flush_by_jump |flush_by_load_use |flush_by_csr|flush_by_ecall_ex|flush_by_ecall_wb|flush_by_mret;
    assign EX_LS_flush      = flush_by_ecall_wb;
    assign LS_WB_flush      = flush_by_ecall_wb;
    ...
    always_comb begin
        ...
        if(flush_by_csr|flush_by_ecall_ex) begin
            pc_set_target = pc_ex+4;
        end
        if (flush_by_ecall_wb) begin
            pc_set_target = mtvec_global;
        end
        if(flush_by_mret) begin
